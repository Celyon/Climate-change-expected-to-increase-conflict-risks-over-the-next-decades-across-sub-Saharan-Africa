...
filename=paste("F:\\Dataset_Simulate_UCDP_Two_Year.csv",sep='')
...
for(i in 1 : 50){
  ...
  modelname<-paste("F:\\UCDP_Two_Year_",i,".Rdata",sep="")
  ...
  simulate_filename <- paste("F:\\UCDP_Two_Year_",i,".csv",sep="")
  write.csv(copy_Alldata, file=simulate_filename,row.names=F)
}
filename<-paste("F:\\UCDP_Two_Year_risk_accuracy.csv",sep="")
