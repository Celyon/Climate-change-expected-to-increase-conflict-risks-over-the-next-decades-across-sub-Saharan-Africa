...
data <- subset(data, select = -Precipitation_Anomaly)
data <- subset(data, select = -Temperature_Anomaly)
data <- subset(data, select = -Mean_Precipitation)
data <- subset(data, select = -Mean_Temperature)
...
data <- data[,1:11]
summary(data)
names(data) <- c('X',	'Y',	'Year',	'Exclusion','GDP','land_cover',	'Liberal_Democracy_Index', 'Accessibility_to_cities', 'Conflict')
...
for(i in 1 : 50){
  ...
  BRT_model <- gbm.step(data=training, gbm.x = 1:5, gbm.y = 6, tree.complexity = 4,learning.rate = 0.01, bag.fraction = 0.75, step.size = 10, cv.folds = 10, max.trees = 10000)
  ...
  modelname<-paste("F:\\Demonstractions_Incidence_NoClimate_",i,".Rdata",sep="")
  ...
  simulate_filename <- paste("F:\\Demonstractions_Incidence_NoClimate_",i,".csv",sep="")
  write.csv(copy_Alldata, file=simulate_filename,row.names=F)
}
filename<-paste("F:\\Demonstractions_Incidence_NoClimate_risk_accuracy.csv",sep="")
